...
export default function Employees() {
  ...
  const toggleActive = async (u) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || "Feilet");
    }
  };
    ...
    <div className="p-6 sm:p-10">
      ...
      <div className="d8-table">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-neutral-500">Laster...</td></tr>
            ) : users.length === 0 ? (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-neutral-500">Ingen ansatte enna.</td></tr>
            ) : users.map(u => {
                ...
                <tr key={u.user_id} className="border-b border-neutral-100 hover:bg-neutral-50" data-testid={`user-row-${u.user_id}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <IconBtn title="Sett nytt passord" onClick={() => setPasswordUser(u)} testId={`password-user-${u.user_id}`}><KeyRound size={14} /></IconBtn>
                      <IconBtn title={u.role === "admin" ? "Gjor til ansatt" : "Gjor til administrator"} disabled={self} onClick={() => toggleRole(u)} testId={`role-${u.user_id}`}>
      ...
      {passwordUser && <PasswordModal user={passwordUser} onClose={() => setPasswordUser(null)} onSaved={() => { setPasswordUser(null); load(); }} />}
    </div>
  );
}
...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  return (
    <UserFormFrame title="Rediger ansatt" subtitle={user.email} onClose={onClose} onSubmit={submit}>
      <UserFields form={form} setForm={setForm} includePassword={false} prefix="edit" />
      <ModalActions onClose={onClose} saving={saving} saveLabel="Lagre" savingLabel="Lagrer..." testId="save-user" />
    </UserFormFrame>
  );
}

function CreateUserModal({ onClose, onSaved }) {
  const [form, setForm] = useState({ username: "", email: "", name: "", employee_number: "", role: "ansatt", password: "" });
  const [saving, setSaving] = useState(false);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || "Kunne ikke opprette bruker");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <UserFormFrame title="Ny bruker" subtitle="Opprett ansatt" onClose={onClose} onSubmit={submit}>
      <UserFields form={form} setForm={setForm} includePassword prefix="create" />
      <ModalActions onClose={onClose} saving={saving} saveLabel="Opprett bruker" savingLabel="Oppretter..." testId="create-user-save" />
    </UserFormFrame>
  );
}
...
function PasswordModal({ user, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.patch(`/users/${user.user_id}/password`, { password });
      toast.success("Passord oppdatert");
      onSaved();
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Kunne ikke sette passord");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <UserFormFrame title="Passord" subtitle={user.name} onClose={onClose} onSubmit={submit}>
      <label className="block">
        <div className="label-eyebrow mb-2">Nytt passord</div>
        <input type="password" className={inputCls} value={password} onChange={e => setPassword(e.target.value)} data-testid="password-user-input" />
      </label>
      <ModalActions onClose={onClose} saving={saving} saveLabel="Lagre passord" savingLabel="Lagrer..." testId="password-user-save" />
    </UserFormFrame>
  );
}
...
function UserFields({ form, setForm, includePassword, prefix }) {
  return (
    <>
      <label className="block">
        <div className="label-eyebrow mb-2">Brukernavn</div>
        <input className={inputCls} value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} data-testid={`${prefix}-user-username`} />
      </label>
      <label className="block">
        <div className="label-eyebrow mb-2">E-post</div>
        <input className={inputCls} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} data-testid={`${prefix}-user-email`} />
      </label>
      <label className="block">
        <div className="label-eyebrow mb-2">Navn</div>
        <input className={inputCls} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} data-testid={`${prefix}-user-name`} />
      </label>
      {includePassword && (
        <label className="block">
          <div className="label-eyebrow mb-2">Passord</div>
          <input type="password" className={inputCls} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} data-testid={`${prefix}-user-password`} />
        </label>
      )}
      <label className="block">
        <div className="label-eyebrow mb-2">Ansattnummer</div>
        <input className={inputCls} value={form.employee_number} onChange={e => setForm({ ...form, employee_number: e.target.value })} placeholder="f.eks. E101" data-testid={`${prefix}-user-empnum`} />
      </label>
      <label className="block">
        <div className="label-eyebrow mb-2">Rolle</div>
        <select className={inputCls} value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} data-testid={`${prefix}-user-role`}>
